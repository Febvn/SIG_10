...
async def update_fasilitas(id: int, item: FasilitasUpdate, pool=Depends(get_db), current_user=Depends(get_current_user)):
    async with pool.acquire() as conn:
        ...
        if item.nama:
            updates.append("nama = $" + str(idx))
            params.append(item.nama)
            idx += 1
        if item.jenis:
            updates.append("jenis = $" + str(idx))
            params.append(item.jenis)
            idx += 1
        if item.alamat:
            updates.append("alamat = $" + str(idx))
            params.append(item.alamat)
            idx += 1
        if item.longitude is not None and item.latitude is not None:
            updates.append("geom = ST_SetSRID(ST_Point($" + str(idx) + ", $" + str(idx + 1) + "), 4326)")
        ...
        query = "UPDATE fasilitas_publik SET " + ", ".join(updates) + " WHERE id = $1"
